...
- Check compliance of Pulumi project with `pulumi preview` against a Policy Pack (without Pulumi Cloud, best on CI/CD):
...
- Enable a Policy Pack for a Pulumi organization with a specific version:

`pulumi policy enable {{organization_name/policy_pack_name}} {{latest|version}}`

- Disable a Policy Pack for a Pulumi organization with a specific version (defaults to all versions):
